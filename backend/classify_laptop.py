...
def classify_laptop_ports(ports_data) -> str:
    """
...
if __name__ == "__main__":
    ...
    with open("laptops_raw.json", "r") as db:
        data = json.load(db)

    for laptop in data:
        if not (laptop_data := laptop["data"]):
            continue
        print(f"[magenta underline]{laptop['name']}[/]")
        if audio_data := laptop_data.get("audio"):
            print(f"[cyan]audio: [/]{classify_laptop_audio(audio_data)}")
        if battery_data := laptop_data.get("battery"):
            print(f"[cyan]battery: [/]{classify_laptop_battery(battery_data)}")
        if camera_data := laptop_data.get("camera"):
            print(f"[cyan]camera: [/]{classify_laptop_camera(camera_data)}")
        if cpu_data := laptop_data.get("cpu"):
            print(f"[cyan]cpu: [/]{classify_laptop_cpu(cpu_data)}")
        if design_data := laptop_data.get("design"):
            print(f"[cyan]design: [/]{classify_laptop_design(design_data)}")
        if display_data := laptop_data.get("display"):
            print(f"[cyan]display: [/]{classify_laptop_display(display_data)}")
        if keyboard_data := laptop_data.get("keyboard"):
            print(
                f"[cyan]keyboard: [/]{classify_laptop_keyboard(keyboard_data)}"
            )
        if measurements_data := laptop_data.get("measurements"):
            print(
                (
                    "[cyan]measurements: [/]"
                    f"{classify_laptop_measurements(measurements_data)}"
                )
            )
        if memory_data := laptop_data.get("memory"):
            print(f"[cyan]memory: [/]{classify_laptop_memory(memory_data)}")
        if network_data := laptop_data.get("network"):
            print(f"[cyan]network: [/]{classify_laptop_network(network_data)}")
        if ports_data := laptop_data.get("ports_interfaces"):
            print(f"[cyan]ports: [/]{classify_laptop_ports(ports_data)}")
        if security_data := laptop_data.get("security"):
            print(
                (
                    "[cyan]security: [/]"
                    f"{classify_laptop_security(security_data)}"
                )
            )
        if software_data := laptop_data.get("software"):
            print(
                f"[cyan]software: [/]{classify_laptop_software(software_data)}"
            )
        if storage_data := laptop_data.get("storage"):
            print(f"[cyan]storage: [/]{classify_laptop_storage(storage_data)}")
        if video_data := laptop_data.get("video"):
            print(f"[cyan]video: [/]{classify_laptop_video(video_data)}")
        """ try:
            input()
        except (EOFError):
            break """
